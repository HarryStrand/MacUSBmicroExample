/*******************************************************
 ...
 Modified by Harry Strand on 2/15/2012 to get access to
 several previously static functions, such as get_vendor_id,
 get_product_id and get_serial_number.
********************************************************/
...
int make_path(IOHIDDeviceRef device, char *buf, size_t len)
{
	int res;
...
int get_transport(IOHIDDeviceRef device, wchar_t *buf, size_t len)
{
...
static void printData(unsigned char *data, int qty)
{
    for (int x = 0; x < qty; x++)
    {
        printf("%x ", data[x]);
...
int main(void)
{
    ...
    unsigned short usbMicroProductId = 0;
    wchar_t usbMicroSerialNbr[256];
    memset(usbMicroSerialNbr, 0, 256);
...
//		printf("Device: %p\t", dev);
//		printf("  %04hx %04hx\t", vendorId, productId);
		...
		printf("  Serial: %ls\t", serial);
		printf("  Loc: %d\t", get_location_id(dev));
        ...
        {
            usbMicroProductId = productId;
            memcpy(usbMicroSerialNbr, serial, 256);
        }
	}
	
    if (usbMicroProductId == 0)
    {
        printf("No USBmicro device attached\n");
        return 1;
    }

    int retVal = 0;

    hid_device *device = hid_open(usbMicroVendorId, usbMicroProductId, usbMicroSerialNbr);
    if (device == NULL)
    {
        printf("Couldn't open usbMicro device with serial number %ls\n", usbMicroSerialNbr);
        return 2;
    }
    ...
    memset(data, 0, 8);
    data[0] = 0xa;  // Port B
    data[1] = 0xff; // All pins
    retVal = hid_write(device, data, 8);
    ...
    while (!done)
    {
        data[0] = 5; // Read port A
        retVal = hid_write(device, data, 8);
//        printf("hid_write returned %d\n", retVal);
        
        memset(data, 0, 256);
        retVal = hid_read(device, data, 8);
        printf("hid_read returned %d - ", retVal);
        printData(data, 8);
        int portAValue = data[1];
        printf("\n");
        
        // Set port B to the same as port A
        memset(data, 0, 8);
        data[0] = 0x2;  // Port B
        data[1] = portAValue; 
        retVal = hid_write(device, data, 8);
